...
- **Reduced tree downstream:** `./script/singularity_run_anaFemtoPhiTreeDownstreamV3.sh MAINCONF TREE.root OUT.root [bufferSize] [mixingMode] [variation] [sigMin] [sigMax] [recomputeDaughterPid] [recomputeMixBin]` — rebuilds phi / phi_rot / phi_mix / h-K candidates and the SE/ME channel histograms from the tree. Driven by the channel list in the maker YAML, so it emits the maker's own channel names. **Use the same mainconf for the tree and the downstream:** the stored flags carry the producer's configuration and the downstream refuses to run on any mismatch of the 119 guarded keys. `singularity_run_anaFemtoPhiTreeDownstream.sh` is the schema 1 reader, kept for older trees; `singularity_dumpFemtoPhiTree.sh` prints a tree's schema, counters and metadata.
