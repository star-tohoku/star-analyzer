...
- **`--generate-joblist`**: Reads mainconf → analysis info and fills **job/joblist/job_template_from_conf.xml**, then writes **`job/joblist/joblist_<anaName>.xml`** using **`analysis.anaName`** for `anaName`. Requires PyYAML; use the **Python 3 environment** above.
...
4. **Submit** from `job/run/`:
   ...
   ```
   If your generated XML uses `__PROJECT_ROOT__`, `submit.sh` will replace it with the actual project path. Log and output URLs come from **analysis.workDir** in the analysis info.

   `submit.sh` now runs preflight before `star-submit`:
   - resolves `anaName` from `joblist_<anaName>.xml`
   - checks `config/mainconf/main_<anaName>.yaml` exists
  ...
  - checks runtime linker resolution in singularity context (fails fast if `ldd` reports `not found`)
   ...
   ```
   This runs `./script/setup.sh config/mainconf/main_<anaName>.yaml && make`, then retries preflight once (use the same **joblist_<anaName>.xml** basename as your `main_<anaName>.yaml`).

   On successful submit, the mainconf and all referenced config files are written to **job/run/configlog/config_<anaName>_<jobid>.txt** (one text file per job) for reproducibility. See `job/run/README.md` for more.
...
2. From the project root, run (requires the **Python 3 environment**):
   ...
   ```
   This fills **job/joblist/job_template_from_conf.xml** from the analysis info and writes **job/joblist/joblist_<anaName>.xml**.
3. Submit from `job/run`: `./submit.sh ../joblist/joblist_<anaName>.xml` (must match **main_<anaName>.yaml** for `submit.sh` preflight).
