void draw_p_y() {
    ...
    // PDFを開く（複数ページの保存を開始）
    c1->Print("rootfile/auau7p3_anaNuclearId/pdf/draw_p_y.pdf[");
    ...
    for (int s = 0; s < nSpecies; ++s) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (h2) {
                TString projName = TString::Format("hY_%s_CentBin%d", sp.Data(), i);
                ...
                title.ReplaceAll("%%", "%");
                h1->SetTitle(TString::Format("%s %s (Y Proj. p < %.1f GeV/c)", sp.Data(), title.Data(), pMaxLabel));
                h1->Draw();
            }
        }
