...
- **FXT 3 GeV official applicability:** STAR's official guidance is that the Run18 Au+Au 3.85 GeV beam-energy (`sqrt(s_NN)=3 GeV`) `fxtmult` parameters are applicable to the P24iy `production_3p85GeV_fixedTarget_2021` data. Their use for the 2021 production is intentional and valid; a separate 2021 parameter table is not required.
- **FXT run coverage:** `fxtmult` index 0 run stop was extended to `22179999` (from `22129999`) so P24iy `production_3p85GeV_fixedTarget_2021` run IDs can initialize with that officially applicable Run18 table. The extension changes only run-ID coverage; the centrality boundaries and correction coefficients are unchanged.
