#!/bin/bash
# Run checkHistAnaLambda.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_checkHistAnaLambda.sh <root_file> <mainconf_path>
# On AL9 login nodes, prefer this wrapper over checkHistAnaLambda.sh (SL7-class STAR runtime).
...
if [[ -z "$ROOT_FILE" || -z "$MAINCONF" ]]; then
  ...
  echo "  root_file     ROOT file produced by run_anaLambda.C" >&2
  echo "  mainconf_path main config (required, e.g. config/mainconf/main_auau19_anaLambda.yaml)" >&2
  exit 1
fi
...
rm -f "$PROJECT_ROOT_REAL"/common/macro/checkHistAnaLambda_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_checkHistAnaLambda.C("$ROOT_FILE_REAL","$ANA_NAME","$MAINCONF_REAL")'
EOF
)
