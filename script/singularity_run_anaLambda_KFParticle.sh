#!/bin/bash
# Run anaLambda_KFParticle.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_run_anaLambda_KFParticle.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
INPUT_FILE="${1:-config/picoDstList/test.list}"
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau19_anaLambda_KFParticle.yaml}"
...
echo "=== anaLambda_KFParticle.C (singularity) ==="
...
# The runner forces an ACLiC rebuild; do not delete cache files with a glob.
...
collect_bind_mounts() {
  ...
  if [[ "$INPUT_FILE_REAL" != *.root && -f "$INPUT_FILE_REAL" ]]; then
...
if [[ -n "$CONFIG_PATH" ]]; then
  ROOT4STAR_CMD="analysis/run_anaLambda_KFParticle.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  ROOT4STAR_CMD="analysis/run_anaLambda_KFParticle.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
