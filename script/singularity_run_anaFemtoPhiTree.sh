#!/bin/bash
# Run anaFemtoPhiTree.C inside singularity (batch-like environment).
# Usage: ./script/singularity_run_anaFemtoPhiTree.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents] [nSkip]
...
export STAR_ANA_GIT_REV="$(git -C "$PROJECT_ROOT_REAL" rev-parse HEAD 2>/dev/null || echo unknown)"
export STAR_ANA_GIT_DIRTY="$(git -C "$PROJECT_ROOT_REAL" status --porcelain 2>/dev/null | grep -v '^??' | wc -l | tr -d ' ')"
export STAR_ANA_LIBRARY_TAG="$LIBRARY_TAG"
rm -f "$PROJECT_ROOT_REAL"/analysis/anaFemtoPhiTree_C.* 2>/dev/null
...
export STAR_ANA_JOBID="$JOBID"
export STAR_ANA_NSKIP="$NSKIP"
export STAR_ANA_GIT_REV="${STAR_ANA_GIT_REV}"
export STAR_ANA_GIT_DIRTY="${STAR_ANA_GIT_DIRTY}"
export STAR_ANA_LIBRARY_TAG="$LIBRARY_TAG"
