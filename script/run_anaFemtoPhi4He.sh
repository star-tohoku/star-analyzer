#!/bin/bash
# Run anaFemtoPhi4He.C - phi-p femtoscopy with StFemtoMaker
# Usage: ./script/run_anaFemtoPhi4He.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
MAINCONF="${MAINCONF:?Usage: ./script/run_anaFemtoPhi4He.sh [MAINCONF] [inputFile] [outputFile] [jobid] [nEvents]}"
...
rm -f "$PROJECT_ROOT"/analysis/anaFemtoPhi4He_C.* 2>/dev/null
root4star -b -q "analysis/run_anaFemtoPhi4He.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$MAINCONF\")"
