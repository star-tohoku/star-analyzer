#!/bin/bash
# Run anaFemtoKaon.C - K- femtoscopy with StFemtoMaker
# Usage: ./script/run_anaFemtoKaon.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
MAINCONF="${MAINCONF:?Usage: ./script/run_anaFemtoKaon.sh [MAINCONF] [inputFile] [outputFile] [jobid] [nEvents]}"
...
rm -f "$PROJECT_ROOT"/analysis/anaFemtoKaon_C.* 2>/dev/null
