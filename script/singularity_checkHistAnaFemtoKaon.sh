#!/bin/bash
# Run checkHistAnaFemtoKaon.C inside singularity.
# Usage: ./script/singularity_checkHistAnaFemtoKaon.sh <root_file> <mainconf_path>
...
rm -f "$PROJECT_ROOT_REAL"/common/macro/checkHistAnaFemtoKaon_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_checkHistAnaFemtoKaon.C("$ROOT_FILE_REAL","$ANA_NAME","$MAINCONF_REAL")'
EOF
)
