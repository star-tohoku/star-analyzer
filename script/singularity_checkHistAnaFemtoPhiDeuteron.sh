#!/bin/bash
# Run checkHistAnaFemtoPhiDeuteron.C inside singularity.
# Usage: ./script/singularity_checkHistAnaFemtoPhiDeuteron.sh <root_file> <mainconf_path>
...
rm -f "$PROJECT_ROOT_REAL"/common/macro/checkHistAnaFemtoPhiDeuteron_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_checkHistAnaFemtoPhiDeuteron.C("$ROOT_FILE_REAL","$ANA_NAME","$MAINCONF_REAL")'
EOF
)
