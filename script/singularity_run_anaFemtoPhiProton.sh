#!/bin/bash
# Run anaFemtoPhiProton.C inside singularity (batch-like environment).
# Usage: ./script/singularity_run_anaFemtoPhiProton.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaFemtoPhiProton_C.* 2>/dev/null
