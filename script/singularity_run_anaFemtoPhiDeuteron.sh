#!/bin/bash
# Run anaFemtoPhiDeuteron.C inside singularity (batch-like environment).
# Usage: ./script/singularity_run_anaFemtoPhiDeuteron.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaFemtoPhiDeuteron_C.* 2>/dev/null
