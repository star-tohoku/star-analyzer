#!/bin/bash
# Run anaFemtoPhiProton.C - phi-p femtoscopy with StFemtoMaker
# Usage: ./script/run_anaFemtoPhiProton.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
MAINCONF="${MAINCONF:?Usage: ./script/run_anaFemtoPhiProton.sh [MAINCONF] [inputFile] [outputFile] [jobid] [nEvents]}"
...
rm -f "$PROJECT_ROOT"/analysis/anaFemtoPhiProton_C.* 2>/dev/null
root4star -b -q "analysis/run_anaFemtoPhiProton.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$MAINCONF\")"
