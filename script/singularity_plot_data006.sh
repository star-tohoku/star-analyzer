#!/bin/bash
# Plot diagnostic CF/acceptance/pair-mT pages from a finalized DATA-006 package.
# Usage: ./script/singularity_plot_data006.sh MAINCONF PACKAGE_DIR OUTDIR [DATA006_CONFIG] [REBIN] [CF_YMIN] [CF_YMAX] [RUN_LABEL] [FILE_PREFIX]
...
MAINCONF="${1:?Usage: $0 MAINCONF PACKAGE_DIR OUTDIR [DATA006_CONFIG] [REBIN] [CF_YMIN] [CF_YMAX] [RUN_LABEL] [FILE_PREFIX]}"
PACKAGE_DIR="${2:?}"
OUT_DIR="${3:?}"
DATA006_CONFIG="${4:-config/maker/data006_auau3p85fxt_pp_pd_dd.yaml}"
DISPLAY_REBIN="${5:-4}"
CF_YMIN="${6:-0.0}"
CF_YMAX="${7:-2.5}"
# The 20k implementation test and a full-statistics delivery must not produce figures that
# claim to be the same run, so the label drawn on every panel and the file prefix are arguments.
RUN_LABEL="${8:-DATA-006 TEST}"
FILE_PREFIX="${9:-data006_test}"
...
PACKAGE_REAL="$(resolve_path "$PACKAGE_DIR")"
OUT_REAL="$(resolve_path "$OUT_DIR")"
DATA006_REAL="$(resolve_path "$DATA006_CONFIG")"
for required in "$MAINCONF_REAL" "$PACKAGE_REAL/data006_correlations.root" "$DATA006_REAL"; do
  if [[ ! -e "$required" ]]; then
...
cd "$PROJECT_ROOT"
root4star -b -q 'tools/run_plotData006Correlations.C("$PACKAGE_REAL","$DATA006_REAL","$OUT_REAL",$DISPLAY_REBIN,$CF_YMIN,$CF_YMAX,"$RUN_LABEL","$FILE_PREFIX")'
EOF
)
