// run_anaFemtoLambda_d.C
void run_anaFemtoLambda_d(const Char_t* inputFile,
                           ...
                           const Char_t* configPath = 0)
{
  ...
  gROOT->ProcessLine(TString::Format(".L %s/analysis/anaFemtoLambda_d.C+", pwd));
  anaFemtoLambda_d(inputFile, outputFile, jobid, nEventsMax, configPath);
}
