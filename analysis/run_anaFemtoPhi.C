// run_anaFemtoPhi.C - Wrapper to load lib and call anaFemtoPhi
// Usage: root4star -b -q 'run_anaFemtoPhi.C("input.list","output.root","0",100)'

void run_anaFemtoPhi(const Char_t* inputFile,
                           ...
                           const Char_t* configPath = 0)
{
  ...
  gROOT->ProcessLine(TString::Format(".L %s/analysis/anaFemtoPhi.C+", pwd));
  anaFemtoPhi(inputFile, outputFile, jobid, nEventsMax, configPath);
}
