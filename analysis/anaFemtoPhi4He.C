// anaFemtoPhi4He.C - StChain based phi-he4 femtoscopy with StFemtoMaker
// Usage: root4star -b -q 'anaFemtoPhi4He.C("input.list","output.root","0",-1)'
// Run from project root: ./script/run_anaFemtoPhi4He.sh
...
void anaFemtoPhi4He(const Char_t* inputFile = "config/picoDstList/auau19GeV.list",
                       const Char_t* outputFile = "rootfile/auau3p85fxt_anaFemtoPhi4He_temp/auau3p85fxt_anaFemtoPhi4He_temp.root",
  ...
  if (configPath && strlen(configPath) > 0) {
    ...
    if (env_conf && strlen(env_conf) > 0) {
      ...
      if (infile.is_open() && std::getline(infile, line)) {
        ...
        mainConfigPath = TString(pwd) + "/config/mainconf/main_auau3p85fxt_anaFemtoPhi4He.yaml";
      }
    }
  }
