// anaFemtoKaon.C - StChain based K- femtoscopy with StFemtoMaker
// Usage: root4star -b -q 'anaFemtoKaon.C("input.list","output.root","0",-1)'
// Run from project root: ./script/run_anaFemtoKaon.sh
...
void anaFemtoKaon(const Char_t* inputFile = "config/picoDstList/auau19GeV.list",
                  const Char_t* outputFile = "rootfile/auau3p85fxt_anaFemtoKaon_temp/auau3p85fxt_anaFemtoKaon_temp.root",
  ...
  if (configPath && strlen(configPath) > 0) {
    ...
    if (env_conf && strlen(env_conf) > 0) {
      ...
      if (infile.is_open() && std::getline(infile, line)) {
        ...
        mainConfigPath = TString(pwd) + "/config/mainconf/main_auau3p85fxt_anaFemtoKaon.yaml";
      }
    }
  }
