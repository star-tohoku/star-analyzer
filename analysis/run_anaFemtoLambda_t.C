// run_anaFemtoLambda_t.C
void run_anaFemtoLambda_t(const Char_t* inputFile,
                           ...
                           const Char_t* configPath = 0)
{
  ...
  gROOT->ProcessLine(TString::Format(".L %s/analysis/anaFemtoLambda_t.C+", pwd));
  anaFemtoLambda_t(inputFile, outputFile, jobid, nEventsMax, configPath);
}
